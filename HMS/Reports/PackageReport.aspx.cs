...
namespace HMS
{
    public partial class PackageReport : SimpleBasePage
    {
        #region Page Events
        ...
        private void ReadyDBList()
        {
           ...
           string query = @"
SELECT UniqueID,Area,C.Address1 as Address,P.PackageName,
...
and (@ServiceProviderID =-1 OR C.ServiceProviderID=@ServiceProviderID)";
           if (ddlCB.SelectedIndex > 0)
            ...
            hTable["@PackageID"] = Common.ToInt(ddlPackage.SelectedValue);
        ...
        private void AddDBListColumns()
        {
            ...
            CustomersDBList.Columns.Add(new Column("PackageName", "Package", 90, "P.PackageName", HorizontalAlign.Left, HorizontalAlign.Center, String.Empty, true));
        ...
        private void FillTotals()
        {
            ...
            int totalCust = objReportBL.PackageReport_GetCustomers(objSetting.OperatorID,
                                                                      Common.ToInt(ddlMSO.SelectedValue)
                                                                      , Common.ToInt(ddlPackage.SelectedValue),
                                                                      UserSetting.UserType, Common.ToInt(ddlCB.SelectedValue));
            lbTotalCustomer.Text = "Total Customers: " + totalCust;
        }
        private void BindPackageDDL(DropDownList ddl)
        {
            DataTable dt = Common.GetDBResult("select PackageID,PackageName from Packages where OperatorID=" + UserSetting.OperatorID);
            ddl.DataSource = dt;
